...
class Compute(object):
    ...
    def __init__(self, soundfile, fs, **options):
        ...
        cutoffs = erb2freq(np.arange(
                ...
                # (freq2erb(high) - freq2erb(low)) / (self.options['num_audio_channels'] + 1) # not sure which one is right
        ))
        ...
        # self.subband_resampled = upfirdn.upfirdn(self.subband_comp, 1, int(self.N/self.w_len))
    ...
    def make_stats(self):
        ...
        self.w_norm = self.w/np.sum(self.w) # normalize the window filter
        self.e_mean = np.sum(self.subband_resampled * self.w_norm, 0)
        env_m0 = self.subband_resampled - self.e_mean
        v2 = np.sum((env_m0 ** 2) * self.w_norm, 0)
        self.e_var = np.sqrt(v2) / self.e_mean
        self.e_skew = np.sum((env_m0 ** 3) * self.w_norm, 0) / (v2 ** 1.5)
        self.e_kurtosis = np.sum((env_m0 ** 4) * self.w_norm, 0) / (v2 ** 2)
...
def apply_filters(x, filters):
    # x = np.reshape(x,x.shape[0],x.shape[1]) # make sure x is one dimensional
    N = np.shape(x)[0]
...
def make_octave_cos_filters(N, fs, num_bands, cutoffs, Q=2):
    ...
    for i in range(0, num_bands):
        ...
        avg = cutoffs[i]
        # filters[l_ind:h_ind, i] = np.cos((f[l_ind:h_ind] - avg) / (h - l) * np.pi) weird numpy indexing!
        filters[l_ind:h_ind+1, i] = np.cos((f[l_ind:h_ind+1] - avg) / (h - l) * np.pi)
        #ipdb.set_trace()
...
def modulation_power(x, filters, w):
    ...
    w = w / np.sum(w)
    x = x.reshape(N, 1) # moved this to avoid taking N fft's instead of one on the whole array
    v = np.sum(w * (x - np.sum(w * x)) ** 2)
    w_matrix = w.repeat(num_bands).reshape(N, num_bands)
    # ipdb.set_trace()
    return np.sum(w_matrix * np.power(mod_x,2),0) / v


def spect(data, winsize=2048):
    wins = [np.hanning(winsize) * data[i * winsize / 2:(i + 2) * winsize / 2] for i in
            xrange(0, len(data) // (winsize / 2) - 1)]
...
if __name__ == "__main__":
    ...
    header, wins, labels = get_features(filenames, 2000, winlen, downsample=1,
                                        # redo = True
                                        )
